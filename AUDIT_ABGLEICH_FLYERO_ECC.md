...
| --- | --- | --- | --- | --- | --- |
| Nur der signierte Stripe-Webhook sollte Zahlungen final auf `PAID` setzen, der aktuelle Stand unterlaeuft diese Regel teilweise. | `src/app/api/stripe/webhook/route.ts` verarbeitet signierte Webhooks serverseitig. Gleichzeitig kann `src/app/api/payments/mock-complete/[id]/route.ts` als eingeloggter Kunde die Zahlung vervollstaendigen und ruft `completePaymentFromCheckoutSession(...)` direkt auf. | bestaetigt | Blocker | Mock-Completion vor offenem Launch entfernen oder hart an expliziten Non-Production-Mode binden. | 1 |
