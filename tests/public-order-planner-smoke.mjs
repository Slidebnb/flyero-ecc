...
const wizardTypes = read("src/app/customer/orders/new/orderWizardTypes.ts");
...
const scheduleStep = read("src/app/customer/orders/new/OrderScheduleStep.tsx");
...
assert.doesNotMatch(homePlannerSearch, /postalCode: selectedForQuery\?\.postalCode \?\? \/\^\\d\{5\}\$\//);
...
assert.match(wizard, /applyLocationResult\(result\);/);
assert.match(wizard, /const retainedSegments = options\?\.forceReplace \? \[\] : areaSegmentsRef\.current/);
assert.match(wizardTypes, /contactPerson\?: string/);
assert.match(wizardTypes, /contactPhone\?: string/);
assert.match(wizardTypes, /notes\?: string/);
assert.match(wizard, /clearLocationSelection/);
assert.match(wizard, /const rawDraft = isPublicPlanner \? null/);
assert.match(wizard, /setItem\(ORDER_DRAFT_KEY/);
...
assert.match(publicGeocode, /PUBLIC_GEOCODE_CITY_MISMATCH/);
assert.match(publicGeocode, /comparablePlace/);
assert.match(read("src/app/api/public/planner/experience/route.ts"), /PUBLIC_GEOCODE_CITY_MISMATCH/);
assert.match(homePlannerSearch, /api\/public\/planner\/geocode/);
assert.match(homePlannerSearch, /resolvingSelection/);
...
assert.match(wizard, /(?:listenerTarget|layer)\.addListener/);
assert.match(wizard, /officialBoundaryListenersRef\.current = \[\]/);
...
assert.doesNotMatch(areaStep, /Fläche auf der Karte zeichnen/);
assert.match(areaStep, /Gebiet auf der Karte auswählen/);
assert.match(materialStep, /automatisch zugewiesene FLYERO-Hauptlager/);
