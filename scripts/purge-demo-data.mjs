...
const checks = [
  ...
  ["Seed-Zahlungsereignisse", Prisma.sql`SELECT count(*)::int AS count FROM "PaymentEvent" WHERE "stripeEventId" LIKE 'evt_seed_%' OR "payload"->>'seed' = 'true'`],
  ["Seed-UX-Ereignisse", Prisma.sql`SELECT count(*)::int AS count FROM "OrderExperienceEvent" WHERE "source" LIKE 'seed.%'`],
  ...
  ["Seed-Lagerregionen", Prisma.sql`SELECT count(*)::int AS count FROM "WarehouseRegion" WHERE "warehouseId" IN (SELECT "id" FROM "Warehouse" WHERE "isDemoData" = true)`],
...
async function purge(tx) {
  const statements = [
    ...
    ["Seed-Zahlungsereignisse", Prisma.sql`DELETE FROM "PaymentEvent" WHERE "stripeEventId" LIKE 'evt_seed_%' OR "payload"->>'seed' = 'true'`],
    ["Seed-UX-Ereignisse", Prisma.sql`DELETE FROM "OrderExperienceEvent" WHERE "source" LIKE 'seed.%'`],
    ...
    ["Seed-Logistik-Auditlogs", Prisma.sql`DELETE FROM "AuditLog" WHERE ("entityType" = 'Order' AND "entityId" IN (SELECT "id" FROM "Order" WHERE ${demoOrderClause})) OR ("entityType" = 'LogisticsShipment' AND "entityId" IN (SELECT "id" FROM "LogisticsShipment" WHERE "notes" ILIKE '%seed%')) OR ("entityType" = 'WarehouseTransfer' AND "entityId" LIKE 'seed-module23-%') OR ("entityType" = 'WarehouseStockCount' AND "entityId" LIKE 'seed-module23-%')`],
    ...
    ["Seed-Auditlogs", Prisma.sql`DELETE FROM "AuditLog" WHERE "action" LIKE 'seed.%' OR "entityId" LIKE 'seed%' OR "newValues"->>'seed' = 'true'`],
    ...
    ["Seed-Lagerregionen", Prisma.sql`DELETE FROM "WarehouseRegion" WHERE "warehouseId" IN (SELECT "id" FROM "Warehouse" WHERE "isDemoData" = true)`],
