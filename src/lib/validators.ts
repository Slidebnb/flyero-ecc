...
export const addressSchema = z.object({
  ...
  postalCode: z.string().trim().regex(/^\d{5}$/, "Bitte gib eine gueltige fuenfstellige PLZ ein."),
  city: z.string().min(1),
...
export const customerProfileUpdateSchema = z.object({
  ...
  billingHouseNumber: optionalText,
  billingPostalCode: z.string().trim().regex(/^\d{5}$/, "Bitte gib eine gueltige fuenfstellige Rechnungs-PLZ ein."),
...
export const orderCreateSchema = z
  .object({
    ...
    targetAreaName: z.string().trim().min(2, "Bitte wähle ein Verteilgebiet aus."),
