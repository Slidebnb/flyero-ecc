...
export function safeCustomerSubject(type: string, subject: string) {
  ...
  return /MODULE|SEED|QUEUE|SMOKE|FIXTURE|TESTDATEN|CUSTOMER_|ADMIN_|DISTRIBUTOR_|DEMO-|Auto-Dispatch|localhost|Touren im Dashboard/i.test(subject) ? mapped : subject;
}

export function safeCustomerMessage(type: string, body: string) {
  if (!/MODULE|SEED|QUEUE|SMOKE|FIXTURE|TESTDATEN|CUSTOMER_|ADMIN_|DISTRIBUTOR_|DEMO-|Auto-Dispatch|localhost|Touren im Dashboard|Auftrag\s+[A-Z0-9-]+/i.test(body)) return body;
