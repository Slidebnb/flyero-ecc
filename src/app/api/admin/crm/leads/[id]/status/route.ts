...
import { changeLeadStatus } from "@/lib/crm";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const lead = await changeLeadStatus(id, await readBody(request), session.id, leadScopeFromSession(session));
    return successResponse(lead);
  } catch (error) {
    if (error instanceof ZodError) return errorResponse("Statuswechsel ist ungültig.", 400);
