...
- Stra√üenzuege, automatische Haushaltsdaten, Live Tracking, PDF, Rechnungen und Stripe bleiben bewusst ausserhalb dieses Moduls.
...
- Notifications werden ausgel√∂st, aber keine E-Mail-Templates gebaut. Modul 15 kann darauf sauber aufsetzen.
...
- Analytics nutzt zentrale Analytics Services in `src/lib/analytics.ts`. Pages und APIs enthalten keine eigene KPI-Logik, sondern nur Auth, Filter√ºbergabe und Darstellung. Dadurch bleiben Berechnungen konsistent zwischen Dashboard, API und CSV Export.
- Das MVP verzichtet bewusst auf externes BI, Data Warehouse und KI-Prognosen. Die Plattform ist noch operativ im Aufbau; ein direktes Read-Modell aus den bestehenden Tabellen ist einfacher pr√ºfbar, schneller iterierbar und reduziert Infrastrukturkomplexit√§t.
- KPIs werden aus operativen Tabellen berechnet: `Order`, `Payment`, `Refund`, `DistributionTour`, `GpsPoint`, `WarehouseInventory`, `DispatchAssignment`, `Report`, `Lead`, `CustomerProfile` und `DistributorProfile`. So entsprechen die Zahlen dem tats√§chlichen Plattformzustand.
- CSV Export ist ein einfacher Admin-only Export der aktuellen Filterdaten. Dadurch k√∂nnen Beta-Auswertungen extern weiterverarbeitet werden, ohne ein neues Reporting-Modul oder externe BI-Anbindung einzuf√ºhren.
...
- FLYERO nutzt im MVP ein eigenes Mini-CRM statt einer externen CRM-Integration. Die Plattform sammelt Leads bereits selbst √ºber Landingpage und Kontaktformulare; ein internes CRM h√§lt Daten, Rollenrechte, AuditLogs und Demo-Seed nah am bestehenden System.
- Die Entscheidung gegen externe CRM-Integration gilt f√ºr das MVP, damit Beta-Vertrieb, Datenschutz und Auditierbarkeit zuerst im eigenen System stabil sind.
- Externe CRM-Integration wird bewusst verschoben. F√ºr die Beta sind Pipeline, Follow-ups, Verantwortliche und Statushistorie wichtiger als Synchronisation mit HubSpot, Pipedrive oder Newsletter-Tools. Dadurch bleiben Datenschutz, Betrieb und Fehleranalyse einfacher.
- Lead-Aktivit√§ten liegen in `LeadActivity`, Gespr√§chsnotizen in `LeadNote`. So bleiben Statuswechsel, Follow-ups, Zuweisungen und Notizen nachvollziehbar, ohne das `Lead`-Modell mit historisierten Textfeldern zu √ºberladen.
- Conversion wird vorbereitet statt voll automatisiert verkauft: Ein gewonnener Lead wird mit einem bestehenden Kunden verkn√ºpft oder als unverifiziertes Kundenkonto angelegt. Dadurch wird `wonCustomerId` gespeichert, aber der Kunde muss sein Konto weiterhin sauber verifizieren.
- CRM-APIs sind admin- bzw. supportbeschr√§nkt. Customer, Distributor und Warehouse erhalten keinen Zugriff auf Leadpipeline oder Vertriebsnotizen.
- Analytics liest CRM-Daten direkt aus `Lead`, `LeadNote` und `LeadActivity`, weil f√ºr die Beta keine separate BI-Schicht n√∂tig ist.
...
- `TicketMessage.visibility` trennt ˆffentliche Kommunikation von internen Notizen. Diese Trennung ist wichtiger als separate Tabellen, weil Admins im selben Verlauf arbeiten, Kunden und Verteiler aber keine internen Pr¸fhinweise sehen d¸rfen.
...
- Anh‰nge sind im MVP datenmodellseitig vorbereitet. Ein produktiver Datei-Upload braucht sp‰ter Storage, Virenscan, Grˆﬂenlimits und Berechtigungspr¸fung pro Download.
- Analytics liest Support-KPIs direkt aus `SupportTicket`, weil f¸r die Beta keine separate Case-Management-Auswertung nˆtig ist.

## Modul 22: DMS, Datei-Storage und Druckmodul

- FLYERO braucht ein echtes Dokumentenmanagement, weil Druckdateien, Versionen, Freigaben, Rechnungen, Berichte und Reklamationsbelege langfristig unterschiedliche Lebenszyklen haben. Ein einzelnes Upload-Feld am Auftrag w√§re nicht auditierbar genug.
- Versionierung liegt in `DocumentVersion`, w√§hrend `Document` immer auf die aktuelle Version zeigt. Dadurch bleiben alte Druckdateien erhalten, ohne Kunden- und Adminseiten kompliziert zu machen.
- Kommentare haben `PUBLIC` und `INTERNAL`, damit Kunden klare Freigabehinweise sehen, Admins aber interne Qualit√§tsnotizen f√ºhren k√∂nnen.
- Storage ist √ºber `src/lib/documentStorage.ts` gekapselt. Das lokale Dateisystem ist nur der MVP-Adapter; S3, Cloudflare R2 oder Azure Blob k√∂nnen sp√§ter hinter derselben Service-Grenze eingebaut werden.
- Downloads laufen √ºber gesch√ºtzte API-Routen statt √∂ffentliche Datei-URLs. Damit bleiben Kundendokumente, Druckdateien und Vertr√§ge rollenbeschr√§nkt.
- Das Druckmodul ist getrennt von der Verteilpreislogik. Druckherstellungspreise werden nicht geraten; `estimatedNetPrice`, `estimatedGrossPrice` und `priceSnapshot` sind f√ºr echte FLYERO-Konditionen vorbereitet.
- `PrintPartner` ist eigenst√§ndig, weil Modul 23 mehrere Lager und optimale Druck-/Logistikwege vorbereiten soll. Druckauftr√§ge sind daher nicht hart auf ein Lager verdrahtet, k√∂nnen aber bei Wareneingang mit `WarehouseInventory` verkn√ºpft werden.
- Der Status `RECEIVED_IN_WAREHOUSE` erzeugt bzw. aktualisiert Lagerbestand und setzt den Auftrag auf `READY_FOR_DISTRIBUTION`. So entsteht der √úbergang vom Druckprozess in Lager und Dispatch ohne manuelle Doppelpflege.
