...
for (const [name, type, city, postalCode, district, households, flyers, distance, lat, lng, radius] of areaSeed) {
  ...
  const area = await prisma.distributionArea.upsert({
    ...
    update: {
      ...
      areaKm2: new Prisma.Decimal((coverageAreaSqm / 1_000_000).toFixed(6)),
      estimatedHouseholds: households,
      estimatedFlyers: flyers,
      estimatedDistanceMeters: distance,
      dataSourceName: "module8-seed",
      dataSourceType: "SEED",
      licenseNote: "Demo-/Seed-Daten, nicht als offizielle Haushaltsdaten verwenden.",
      dataUpdatedAt: new Date("2026-07-10T00:00:00.000Z"),
  ...
  if (!existingEstimate) {
    await prisma.areaHouseholdEstimate.create({
      data: {
        ...
        sourceYear: 2026,
        confidence: new Prisma.Decimal("0.700"),
        notes: "Demo-/Seed-Daten, nicht als offizielle Haushaltsdaten verwenden.",
...
const module27Templates = [
  ...
  ["LOGISTICS_CUSTOMER_DELIVERY_EXPECTED", "CUSTOMER", "Lieferadresse fuer deine Flyer", "Lieferadresse fuer {{orderNumber}}", "Deine Flyerlieferung fuer {{orderNumber}} ist vorbereitet. Lager: {{warehouseName}}. Lieferadresse: {{warehouseAddress}}. Menge: {{flyerQuantity}} Flyer. Paketreferenz: {{packageReference}}. {{nextStep}}"],
